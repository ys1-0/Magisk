...
- [General]使用最小的libc来构建静态可执行文件(`magiskinit`和`magiskboot`)对于较小尺寸
- [核心]为魔术安装拆下不必要的镜子
...
- [App]主要内部代码重构
- [App]支持为大于8GiB的图像修补三星固件
- [App]在Android14上使用用户启动的作业而不是前台服务
- [App]支持Android13+内置每个应用程序的语言首选项
- [App]添加`action.sh`支持允许模块定义从UI触发的操作
- [MagiskBoot]支持拆分内核映像，无需解压缩
- [MagiskBoot]正确支持供应商启动映像
- [MagiskBoot]从内核映像禁用三星Proca
